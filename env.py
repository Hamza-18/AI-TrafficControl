...
class Enviornment:
    ...
    def get_state(self):
        ...
        if self.wait_traffic_flow_top_right > 25:
            traffic_flow_top_right = "high"
        elif self.wait_traffic_flow_top_right > 10 and self.wait_traffic_flow_top_right < 25:
            traffic_flow_top_right = "med"
        else:
            traffic_flow_top_right = "low"
        
        if self.wait_traffic_flow_bot_left > 25:
            traffic_flow_bot_left = "high"
        elif self.wait_traffic_flow_bot_left > 10 and self.wait_traffic_flow_bot_left < 25:
            traffic_flow_bot_left = "med"
        else:
            traffic_flow_bot_left = "low"

        if self.wait_traffic_flow_bot > 25:
            traffic_flow_bot = "high"
        elif self.wait_traffic_flow_bot > 10 and self.wait_traffic_flow_bot < 25:
    ...
    def get_reward(self, new_state, old_state):
        ...
        if current_phase not in ["GrG", "Gry", "GGr", "Gyr"]:
            self.reward -= 25  # Penalize invalid phases
        return self.reward
