...
if args.gui:
    traci.start(["venv/bin/sumo-gui", "-n", "intersection.net.xml","-r", "intersection.rou.xml", "--start", "--time-to-teleport", "-1"])  # Use sumo-gui for GUI mode
else:
...
for i in range(10):
    ...
    while step < 15000:
        ...
        agent.learn(new_state, env.get_reward(new_state, state, action))
        step +=1
    env.reset()
